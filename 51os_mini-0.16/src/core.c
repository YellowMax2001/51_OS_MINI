...
unsigned char task_create(unsigned char priority, unsigned int task_address, unsigned char xdata *task_msg, unsigned char *name)
{
	...
	for(i = 0; i < TASK_MAX; i ++)
	{
		...
		{
#ifdef XDATA_SP_BUFFER 	//栈空间在xdata区
			/* 初始化清空堆栈区 */
...
#endif
#ifdef IDATA_SP_BUFFER 	//栈空间在idata区
...
//	push_ptr();	 /* 在现有编译环境下寄存器已经由系统自动保存了 */
	...
	for(temp = 0; temp < TASK_MAX; temp ++)
	{
		if(task_run_flag & (1 << temp))
		{
			...
			{
				if(task_pcb_buf[temp].delay != 0) 	//任务在延时，不切换，任务延时数量减一
				{
					task_pcb_buf[temp].delay --;			
				}
				else if(task_pcb_buf[temp].suspend == 1) 	//任务被挂起，不切换
					continue;
				else
				{
					current_id = temp;		
...
void task_delay(unsigned char delay)
{
	...
	TF1 = 1;	//设置完之后立马进入中断，否则任务会继续执行到下一个中断自行到来
}
